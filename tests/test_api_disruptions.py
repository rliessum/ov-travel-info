...
from custom_components.ret_ns_departures.const import NS_DISRUPTIONS_API_BASE_URL
...
def test_parse_disruptions_minimal(disruptions_client):
    ...
    raw = [
        {
            ...
            "timespans": [
                {
                    "cause": {"label": "Signalling"},
                    "situation": {"label": "No trains"},
                    "additionalTravelTime": {"label": "+15 min"},
                }
            ],
            "description": "Signal failure between stations",
            "period": "morning",
    ...
    assert d["situation"] == "No trains"
    assert d["additional_travel_time"] == "+15 min"
    assert d["description"] == "Signal failure between stations"
...
def test_parse_disruptions_skips_without_type(disruptions_client):
    ...
    assert not disruptions_client._parse_disruptions(raw)
...
async def test_async_get_disruptions_success(disruptions_client, mock_session):
    ...
    assert args[0] == NS_DISRUPTIONS_API_BASE_URL
...
@pytest.mark.asyncio
async def test_async_get_disruptions_falls_back_to_reisinformatie(
    disruptions_client, mock_session
):
    """If disruptions-api v3 is not in the subscription, use Reisinformatie v3."""
    ...
    error_cm = MagicMock()
    error_cm.__aenter__ = AsyncMock(side_effect=denied)
    ...
    result = await disruptions_client.async_get_disruptions(station_code="Rtd")

    assert result[0]["id"] == "fb"
    assert mock_session.get.call_count == 2
    assert "/reisinformatie-api/api/v3/disruptions" in mock_session.get.call_args[0][0]
