...
**State**: Timestamp of the next departure that will actually run (`device_class: timestamp`, e.g., `2024-11-16T10:32:00+01:00`). Cancelled departures are skipped; if no departure remains, the state is `unknown`. Home Assistant renders timestamp sensors as relative time ("in 8 minutes") on dashboards.

**Name**: Updates to the next service, e.g. `IC 2834 to Amsterdam Centraal` or `Line 2 to Nesselande` (Dutch HA: `naar` / `Lijn`).
...
description: "Line 2 to Nesselande"
...
**State**: Minutes until the next non-cancelled departure (integer, e.g., `8`)
...
**Attributes**: Same as Next Departure Sensor, minus the `departures` list
...
**Attributes**: `disruptions` (list), `count`, `station_name`, plus `latitude` / `longitude` and `geojson` from [Spoorkaart getStoring](https://apiportal.ns.nl/api-details#api=spoorkaart-api&operation=getStoring) when the rail map has geometry — see [features/ns-disruptions.md](features/ns-disruptions.md).
