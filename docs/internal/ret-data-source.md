...
- Stop IDs now use friendly names (e.g., "beurs", "rotterdam-centraal") instead of NL:Q: codes
...
- **New format**: `beurs`, `rotterdam-centraal`, `provenierssingel`

Users will need to update their stop IDs to use the friendly names that appear in RET's website URLs.

### Example Stop Names
- Beurs: `beurs`
- Rotterdam Centraal: `rotterdam-centraal` (not `centraal-station` — that page is out of service)
- Provenierssingel: `provenierssingel` (replacement for Schiekade during Hofplein works)
- Confirm current stops on [Dienstregeling](https://www.ret.nl/)
